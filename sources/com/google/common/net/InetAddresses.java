...
public final class InetAddresses {
    ...
    private static byte[] textToNumericFormatV6(String str) {
        ...
        while (i6 < i) {
            try {
                allocate.putShort(parseHextet((String) splitToList.get(i6)));
                i6++;
            } catch (NumberFormatException unused) {
                return null;
            }
        ...
        while (i2 > 0) {
            allocate.putShort(parseHextet((String) splitToList.get(splitToList.size() - i2)));
            i2--;
        }
    ...
    public static InetAddress increment(InetAddress inetAddress) {
        ...
        Preconditions.checkArgument(z, "Incrementing %s would wrap.", (Object) inetAddress);
        address[length] = (byte) (address[length] + 1);
        return bytesToInetAddress(address);
    }
