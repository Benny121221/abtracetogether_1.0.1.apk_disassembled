...
public abstract class ConnectableObservable<T> extends Observable<T> {
    ...
    private ConnectableObservable<T> onRefCount() {
        return this instanceof ObservablePublishClassic ? RxJavaPlugins.onAssembly((ConnectableObservable<T>) new ObservablePublishAlt<T>(((ObservablePublishClassic) this).publishSource())) : this;
    }

    @CheckReturnValue
    @SchedulerSupport("none")
    public Observable<T> refCount() {
        return RxJavaPlugins.onAssembly((Observable<T>) new ObservableRefCount<T>(onRefCount()));
    }
    ...
    public final Observable<T> refCount(int i, long j, TimeUnit timeUnit, Scheduler scheduler) {
        ...
        ObservableRefCount observableRefCount = new ObservableRefCount(onRefCount(), i, j, timeUnit, scheduler);
