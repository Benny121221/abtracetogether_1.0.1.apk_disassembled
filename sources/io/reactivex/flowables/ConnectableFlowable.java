...
public abstract class ConnectableFlowable<T> extends Flowable<T> {
    ...
    private ConnectableFlowable<T> onRefCount() {
        ...
        return RxJavaPlugins.onAssembly((ConnectableFlowable<T>) new FlowablePublishAlt<T>(flowablePublishClassic.publishSource(), flowablePublishClassic.publishBufferSize()));
    }
    ...
    public Flowable<T> refCount() {
        return RxJavaPlugins.onAssembly((Flowable<T>) new FlowableRefCount<T>(onRefCount()));
    }
    ...
    public final Flowable<T> refCount(int i, long j, TimeUnit timeUnit, Scheduler scheduler) {
        ...
        FlowableRefCount flowableRefCount = new FlowableRefCount(onRefCount(), i, j, timeUnit, scheduler);
