...
public final class CommandHandler extends Handler {
    ...
    public void handleMessage(Message message) {
        if (message != null) {
            ...
            if (bluetoothMonitoringService != null) {
                bluetoothMonitoringService.runService(Command.Companion.findByValue(i));
            }
        }
    }

    public final void sendCommandMsg(Command command, long j) {
        Intrinsics.checkParameterIsNotNull(command, "cmd");
        sendMessageDelayed(Message.obtain(this, command.getIndex()), j);
    }

    public final void sendCommandMsg(Command command) {
        Intrinsics.checkParameterIsNotNull(command, "cmd");
    ...
    public final void scheduleNextScan(long j) {
        cancelNextScan();
        sendCommandMsg(Command.ACTION_SCAN, j);
    }

    public final void cancelNextScan() {
        removeMessages(Command.ACTION_SCAN.getIndex());
    }

    public final boolean hasScanScheduled() {
        return hasMessages(Command.ACTION_SCAN.getIndex());
    }

    public final void scheduleNextAdvertise(long j) {
        cancelNextAdvertise();
        sendCommandMsg(Command.ACTION_ADVERTISE, j);
    }

    public final void cancelNextAdvertise() {
        removeMessages(Command.ACTION_ADVERTISE.getIndex());
    }

    public final boolean hasAdvertiseScheduled() {
        return hasMessages(Command.ACTION_ADVERTISE.getIndex());
    }
}
