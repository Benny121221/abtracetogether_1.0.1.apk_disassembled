...
final /* synthetic */ class FlowKt__ContextKt {
    ...
    public static final <T> Flow<T> buffer(Flow<? extends T> flow, int i) {
        Intrinsics.checkParameterIsNotNull(flow, "$this$buffer");
        if (!(i >= 0 || i == -2 || i == -1)) {
            ...
            ChannelFlowOperatorImpl channelFlowOperatorImpl = new ChannelFlowOperatorImpl(flow, null, i, 2, null);
            return channelFlowOperatorImpl;
        }
    }

    public static final <T> Flow<T> conflate(Flow<? extends T> flow) {
        Intrinsics.checkParameterIsNotNull(flow, "$this$conflate");
        return FlowKt.buffer(flow, -1);
    }

    public static final <T> Flow<T> flowOn(Flow<? extends T> flow, CoroutineContext coroutineContext) {
        Intrinsics.checkParameterIsNotNull(flow, "$this$flowOn");
        ...
        ChannelFlowOperatorImpl channelFlowOperatorImpl = new ChannelFlowOperatorImpl(flow, coroutineContext, 0, 4, null);
        return channelFlowOperatorImpl;
    }
    ...
    public static final <T, R> Flow<R> flowWith(Flow<? extends T> flow, CoroutineContext coroutineContext, int i, Function1<? super Flow<? extends T>, ? extends Flow<? extends R>> function1) {
        Intrinsics.checkParameterIsNotNull(flow, "$this$flowWith");
        Intrinsics.checkParameterIsNotNull(coroutineContext, "flowContext");
